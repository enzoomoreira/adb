...
class ExpectationsClient:
    ...
    def get_selic_expectations(
        ...
        """
        Busca expectativas de Selic (endpoint especializado).
    ...
    def get_annual_expectations(
        ...
        """
        Busca expectativas anuais para um indicador.

        Args:
            indicator: Nome do indicador ('IPCA', 'PIB Total', 'Cambio', etc)
            reference_year: Ano de referencia da projecao
            start_date: Data inicial da pesquisa
            limit: Limite de registros
            top5: Se True, usa endpoint Top5 (melhores previsores)

        Returns:
            DataFrame com expectativas anuais
        """
        endpoint = 'top5_anuais' if top5 else 'anuais'
        return self.query(
    ...
    def get_inflation_expectations(
        ...
        """
        Busca expectativas de inflacao suavizada (12 ou 24 meses).

        Args:
            indicator: Indicador de inflacao ('IPCA', 'IGP-M', etc)
            horizon: Horizonte em meses (12 ou 24)
            start_date: Data inicial
            limit: Limite de registros

        Returns:
            DataFrame com expectativas de inflacao
        """
        endpoint = f'inflacao_{horizon}m'
        return self.query(
            endpoint_key=endpoint,
            indicator=indicator,
            start_date=start_date,
    ...
    def get_monthly_expectations(
        ...
        """
        Busca expectativas mensais para um indicador.
        ...
        Returns:
            DataFrame com expectativas mensais
        """
        return self.query(
            endpoint_key='mensais',
            indicator=indicator,
            start_date=start_date,
            limit=limit,
        )
