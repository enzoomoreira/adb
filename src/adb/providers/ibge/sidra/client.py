...
class SidraClient:
    ...
    def get_data(
        ...
        Args:
            config: Dict do indicador com parameters, frequency.
    ...
    def _format_data(self, data: list, params: dict) -> pd.DataFrame:
        ...
        try:
            ...
            if isinstance(data, list) and len(data) > 0 and "resultados" in data[0]:
                ...
                df = pd.DataFrame(
                    list(series_data.items()), columns=["date_raw", "value"]
                )
            ...
            if frequency == "quarterly":
                ...
                try:
                    ...
                    df["date_period"] = pd.PeriodIndex(
                        df["year"] + "Q" + df["quarter"].astype(str), freq="Q"
                    )
                    ...
                    df["date"] = pd.to_datetime(
                        df["date_raw"], errors="coerce", format="%Y%m"
                    )
