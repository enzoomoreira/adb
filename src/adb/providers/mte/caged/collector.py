...
class CAGEDCollector(BaseCollector):
    ...
    def _fetch_single_period(
        ...
        try:
            ...
            csv_path = next(
                (f for f in csv_files if config["prefix"] in f.name.upper()),
                csv_files[0],
            )
            ...
            try:
                ...
                conn.execute(query)
                result = conn.execute(
                    f"SELECT COUNT(*) FROM '{str(output_path)}'"
                ).fetchone()
    ...
    def collect(
        ...
        for key in keys:
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                ...
                future_to_period = {
                    executor.submit(self._fetch_single_period, key, year, month): (
                        year,
                        month,
                    )
            ...
            if errors:
                self.logger.warning(
                    f"  {len(errors)} periodo(s) falharam: {', '.join(errors[:3])}"
                )
    ...
    def get_status(self, subdir: str | None = None) -> pd.DataFrame:
        ...
        for key in CAGED_CONFIG.keys():
            ...
            try:
                ...
                total, min_d, max_d = res if res else (0, None, None)
                ...
                first_date = (
                    pd.to_datetime(str(min_d), format="%Y%m") if min_d else None
                )
